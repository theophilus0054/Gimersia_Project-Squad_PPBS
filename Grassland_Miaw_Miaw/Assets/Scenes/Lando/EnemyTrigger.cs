...
{
    private EnemyAttack parentAttack;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        Debug.Log($"Target Seen: {other.name} & Tag: {other.tag}");


        if (other.TryGetComponent<IDamageable>(out var dmg) && other.CompareTag("Creature") && !other.GetComponent<Creature>().IsDead)
        {
            Debug.Log($"Creature Seen: {other.name}");
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        ...
        if (parentAttack.enemiesInRange.Contains(other.transform))
        {
            Debug.Log($"Dadah: {other.name} & Tag: {other.tag}");
